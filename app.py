...
def choose_best_image(*candidates):
    blacklist = ("sprite", "logo", "icon", "avatar", "banner-small", "placeholder", "gravatar")
    valid = []
...
def _extract_image_from_jsonld_obj(obj, base_url=""):
    if isinstance(obj, dict):
        for key in ("image", "thumbnailUrl", "contentUrl", "url"):
            value = obj.get(key)
            if isinstance(value, str):
                image_url = choose_best_image(value)
                if image_url:
                    return normalize_image_url(image_url, base_url)
            elif isinstance(value, list):
                for item in value:
                    found = _extract_image_from_jsonld_obj(item, base_url)
                    if found:
                        return found
            elif isinstance(value, dict):
                found = _extract_image_from_jsonld_obj(value, base_url)
                if found:
                    return found
        ...
        image_url = choose_best_image(obj)
        if image_url:
            return normalize_image_url(image_url, base_url)
    return ""
...
def extract_image_from_entry(entry):
    ...
    summary_html = entry.get("summary", "") or entry.get("description", "") or entry.get("content", [{"value":""}])[0].get("value", "")
    if summary_html:
        soup = BeautifulSoup(summary_html, "html.parser")
        for img in soup.find_all(["img", "source"], limit=8):
            image_url = choose_best_image(
                ...
                img.get("data-layzr"),
                first_src_from_srcset(img.get("srcset", "")),
                first_src_from_srcset(img.get("data-srcset", "")),
            )
            if image_url:
                return image_url

    media_credit = entry.get("media_credit") or entry.get("image") or entry.get("featured_image")
    if isinstance(media_credit, str):
        image_url = choose_best_image(media_credit)
        if image_url:
            return image_url
...
def extract_image_from_html(soup, raw_html="", base_url=""):
    ...
    for img in soup.find_all(["img", "source", "video"], limit=30):
        image_url = choose_best_image(
            img.get("src"),
            img.get("poster"),
            img.get("data-src"),
    ...
    for noscript in soup.find_all("noscript", limit=10):
        nested = BeautifulSoup(noscript.get_text(" ", strip=True), "html.parser")
        nested_img = nested.find("img")
        if nested_img:
            image_url = choose_best_image(
                nested_img.get("src"),
                nested_img.get("data-src"),
                first_src_from_srcset(nested_img.get("srcset", "")),
            )
            if image_url:
                return normalize_image_url(image_url, base_url)
    ...
    for script in soup.find_all("script", {"type": "application/ld+json"}):
        try:
            ...
            image_url = _extract_image_from_jsonld_obj(payload, base_url)
            if image_url:
                return image_url
        except Exception:
            pass
...
def canonicalize_image_key(image_url):
    image_url = normalize_image_url(image_url or "")
    ...
    path = re.sub(r"@\d+x(?=\.[a-z0-9]+$)", "", path)
    basename = path.rsplit("/", 1)[-1]
    return f"{parsed.netloc.lower()}/{basename}" if basename else path


def title_similarity(a, b):
    a_norm = normalize_for_dedupe(a)
    b_norm = normalize_for_dedupe(b)
...
def text_similarity(a, b):
    a_norm = clean_text(a).lower()[:600]
    b_norm = clean_text(b).lower()[:600]
    if not a_norm or not b_norm:
        return 0.0
    return SequenceMatcher(None, a_norm, b_norm).ratio()
...
def content_fingerprint(item):
    title = normalize_for_dedupe(item.get("title", ""))
    summary = normalize_for_dedupe(item.get("summary", ""))
    article = normalize_for_dedupe((item.get("fetched_article_text", "") or "")[:400])
    image_key = canonicalize_image_key(item.get("image_url", ""))
    payload = " | ".join(part for part in [title, summary[:180], article[:180], image_key] if part)
    if not payload:
        payload = canonicalize_url(item.get("link", ""))
    return hashlib.md5(payload.encode("utf-8", errors="ignore")).hexdigest()


def prefers_item(candidate, incumbent):
    cand_has_real_image = bool(canonicalize_image_key(candidate.get("image_url", "")))
    inc_has_real_image = bool(canonicalize_image_key(incumbent.get("image_url", "")))
    cand_summary_len = len(clean_text(candidate.get("summary", ""))) + len(clean_text(candidate.get("fetched_article_text", "")))
    inc_summary_len = len(clean_text(incumbent.get("summary", ""))) + len(clean_text(incumbent.get("fetched_article_text", "")))
    cand_pub = parse_date(candidate.get("published_at")) if candidate.get("published_at") else None
    inc_pub = parse_date(incumbent.get("published_at")) if incumbent.get("published_at") else None

    cand_rank = (
        candidate.get("score", 0),
        1 if cand_has_real_image else 0,
        cand_summary_len,
        1 if cand_pub else 0,
        -(abs((datetime.now(timezone.utc) - cand_pub).total_seconds()) if cand_pub else 10**12),
    )
    inc_rank = (
        incumbent.get("score", 0),
        1 if inc_has_real_image else 0,
        inc_summary_len,
        1 if inc_pub else 0,
        -(abs((datetime.now(timezone.utc) - inc_pub).total_seconds()) if inc_pub else 10**12),
    )
    return cand_rank > inc_rank


def are_likely_duplicates(item, other, overlap_threshold=SIMILARITY_THRESHOLD):
    if canonicalize_url(item.get("link", "")) and canonicalize_url(item.get("link", "")) == canonicalize_url(other.get("link", "")):
        return True, "same_link"

    item_fp = item.get("_content_fingerprint") or content_fingerprint(item)
    other_fp = other.get("_content_fingerprint") or content_fingerprint(other)
    if item_fp == other_fp:
        return True, "same_fingerprint"

    item_title = item.get("title", "")
    other_title = other.get("title", "")
    t_sim = title_similarity(item_title, other_title)
    if t_sim >= TITLE_SIMILARITY_THRESHOLD:
        return True, f"title_sim={t_sim:.2f}"

    item_img = canonicalize_image_key(item.get("image_url", ""))
    other_img = canonicalize_image_key(other.get("image_url", ""))
    if item_img and other_img and item_img == other_img:
        combined_a = f"{item_title} {item.get('summary', '')}"
        combined_b = f"{other_title} {other.get('summary', '')}"
        combo_sim = text_similarity(combined_a, combined_b)
        if combo_sim >= 0.58:
            return True, f"same_image+text={combo_sim:.2f}"

    item_kw = item.get("_kw_overlap") or keyword_set_for_overlap(item)
    other_kw = other.get("_kw_overlap") or keyword_set_for_overlap(other)
    sim = jaccard_similarity(item_kw, other_kw)
    if sim >= overlap_threshold:
        return True, f"overlap={sim:.2f}"

    article_a = item.get("fetched_article_text", "") or item.get("summary", "")
    article_b = other.get("fetched_article_text", "") or other.get("summary", "")
    txt_sim = text_similarity(article_a, article_b)
    if txt_sim >= TEXT_SIMILARITY_THRESHOLD and t_sim >= 0.62:
        return True, f"text_sim={txt_sim:.2f}"
...
def dedupe_news(news):
    prepared = []
    for item in news:
        item["_kw_overlap"] = keyword_set_for_overlap(item)
        item["_content_fingerprint"] = content_fingerprint(item)
    ...
    for item in prepared:
        ...
        for idx, kept_item in enumerate(kept):
            is_dup, reason = are_likely_duplicates(item, kept_item)
            if is_dup:
                duplicate_index = idx
        ...
        if prefers_item(item, incumbent):
            print(f"  [dedupe primário {duplicate_reason}] substituída: '{incumbent['source']}' -> '{item['source']}' | {item['title'][:70]}")
            kept[duplicate_index] = item
        else:
            print(f"  [dedupe primário {duplicate_reason}] descartada: '{item['source']}' | {item['title'][:70]}")

    return kept


def dedupe_by_content_overlap(news, threshold=SIMILARITY_THRESHOLD):
    """Segunda passagem de deduplicação baseada em conteúdo enriquecido.
    Remove notícias que cobrem o mesmo evento com títulos diferentes ou mesma foto.
    Mantém sempre a melhor peça editorial.
    """
    ...
    for item in sorted_news:
        ...
        item.setdefault("_kw_overlap", keyword_set_for_overlap(item))
        item.setdefault("_content_fingerprint", content_fingerprint(item))

        for idx, kept_item in enumerate(kept):
            is_dup, reason = are_likely_duplicates(item, kept_item, overlap_threshold=threshold)
            if is_dup:
                duplicate_index = idx
                duplicate_reason = reason
                break
        ...
        incumbent = kept[duplicate_index]
        if prefers_item(item, incumbent):
            print(f"  [dedupe final {duplicate_reason}] substituída: '{incumbent['source']}' -> '{item['source']}' | {item['title'][:70]}")
            kept[duplicate_index] = item
        else:
            print(f"  [dedupe final {duplicate_reason}] descartada: '{item['source']}' | {item['title'][:70]}")
...
@media (max-width: 720px){{.back-btn{{left:20px;top:18px;padding:8px 12px;font-size:.78rem;}}.header-inner{{padding:64px 20px 24px;flex-direction:column;align-items:flex-start;}}.header-meta{{text-align:left;}}.header-controls{{justify-content:flex-start;}}.wrap{{padding:24px 20px 50px;}}.card{{grid-template-columns:1fr;}}.thumb-wrap{{aspect-ratio:16/9;}}.site-footer{{padding:0 20px 36px;}}}}
</style>
</head>
<body>
