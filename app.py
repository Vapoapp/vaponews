...
def save_memory(memory):
    try:
        ...
        with open(MEMORY_FILE, "w", encoding="utf-8") as f:
            json.dump(memory[-MEMORY_MAX_DAYS:], f, ensure_ascii=False, indent=2)
    except Exception:
        pass
...
def split_ai_column(content):
    raw = clean_text(content or "")
    if not raw:
        return "🧠 IA VAPO — Radar do Dia", "", ""
    lines = [clean_text(x) for x in (content or "").splitlines() if clean_text(x)]
    if lines:
        first = lines[0]
...
def build_html(grouped, ai_column=None):
    ...
    ai_column = ai_column or {"title": "🧠 IA VAPO — Radar do Dia", "preview": "Coluna da IA indisponível nesta execução.", "body": "Coluna da IA indisponível nesta execução.", "drivers": []}
    ai_title = html.escape(ai_column.get("title", "🧠 IA VAPO — Radar do Dia"))
...
<main><div class="wrap">
  <section class="ai-column">
    <div class="ai-column-head">
      <div class="ai-column-title">{ai_title}</div>
      <div class="ai-column-sub">Leitura editorial curta com base nas notícias selecionadas</div>
    </div>
    <div class="ai-pills">{ai_drivers}</div>
    <div class="ai-column-body">
      <p class="ai-preview" id="ai-preview">{ai_preview}</p>
      <div class="ai-full" id="ai-full">{ai_body.replace("\n", "<br><br>")}</div>
    </div>
...
function toggleAI(){{const full=document.getElementById('ai-full');const btn=document.getElementById('aiToggleBtn');if(!full||!btn)return;const open=full.classList.toggle('open');btn.textContent=open?'Mostrar menos':'Ler mais';}}
async function shareAIColumn(){{const title=document.querySelector('.ai-column-title')?.innerText||'🧠 IA VAPO — Radar do Dia';const preview=document.getElementById('ai-preview')?.innerText||'';const full=document.getElementById('ai-full')?.innerText||preview;const text=`${{title}}

${{full||preview}}`;if(navigator.share){{try{{await navigator.share({{title, text}});return;}}catch(e){{}}}}try{{await navigator.clipboard.writeText(text);alert('Texto da coluna da IA copiado para compartilhar.');}}catch(e){{window.prompt('Copie o texto da coluna da IA:', text);}}}}
