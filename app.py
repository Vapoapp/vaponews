...
def fetch_brasil_energia(session, url, base_url, cache=None):
    ...
    soup = BeautifulSoup(response.text[:350000], "html.parser")
    items = []
    seen = set()
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not href or not full_link.startswith(base_url):
            continue

        if "/petroleoegas/" not in full_link:
            continue

        if any(block in full_link.rstrip("/").lower() for block in [
            "/ultimasnoticias", "/petroleoegas/", "/categoria/", "/tag/", "/author/", "/page/"
        ]):
            continue
...
GENERIC_SOURCES = {
    ...
    "hot topics maritime",
    "infomoney mundo",
    "click petróleo e gás", "click petroleo e gas"
}
...
def fetch_source(source, cache=None):
    ...
    try:
        ...
        if source_name in {"brasil energia", "brasil energia petróleo e gás", "brasil energia petroleo e gas"}:
